...
DFC_START_SOC = int(os.getenv("DFC_START_SOC", "60"))          # %
TIME_MARGIN_MIN = int(os.getenv("TIME_MARGIN_MIN", "15"))      # min
...
def DFC(data, collect_stats=False):
    ...
    # ===== DEBUG =====
    print(f"\n[DEBUG] DFC_START_SOC={DFC_START_SOC}")
...
def _collect_input_files(input_folder, pattern="*.csv"):
    ...
    if DEBUG_DEVICE is not None:
        files = [p for p in files if DEBUG_DEVICE in p.name]
...
if __name__ == "__main__":
    ...
    input_folder_path = str(
        root_dir / f"R_parsing_origin_{VEHICLE_MODEL}"
    )

    output_root = (
        root_dir / f"DFC_origin_{VEHICLE_MODEL}"
    )

    output_folder_path = str(
        output_root / DFC_SUFFIX.lstrip("_")
    )

    summary_root = (
        root_dir / f"DFC_summary_{VEHICLE_MODEL}"
    )

    summary_folder_path = str(
        summary_root / DFC_SUFFIX.lstrip("_")
    )
    ...
    summary_csv_path = os.path.join(
        summary_folder_path,
        f"dfc_features_summary{DFC_SUFFIX}_{Path(input_folder_path).name}.csv"
    )
