...
LOG_PATH = (
    OUTPUT_DIR
    / f"temperature_manipulation_log{DFC_SUFFIX}.txt"
)
...
STATUS_MANIPULATED = "Manipulated"

STATUS_NOT_MANIPULATED = (
    "Not_manipulated_drive_temp_not_lower"
)
...
def calculate_module_average(temp_value) -> float:
    """mod_temp_list 한 행에서 모듈 평균 온도를 계산한다."""
...
def create_temperature_columns(
    ...
    """
    mod_temp_manipulated_avg를 생성하고,
    ...
    # 각 시간 행의 모듈 평균 온도를 새로 계산한다.
    df["mod_temp_manipulated_avg"] = (
        df["mod_temp_list"]
        .apply(calculate_module_average)
    )
...
def manipulate_temperature(
    ...
    주행 시작 온도가 더 낮으면 DFC 구간 전체의
    mod_temp_avg를 두 온도의 차이만큼 낮춘다.
    """
    ...
    # DFC 충전 재개 시점의 모듈 평균 온도
    dfc_start_temp = pd.to_numeric(
        pd.Series([
            df.loc[start_idx, "mod_temp_manipulated_avg"]
        ]),
        errors="coerce",
    ).iloc[0]
    ...
    drive_start_temp = pd.to_numeric(
        pd.Series([
            df.loc[drive_start_idx, "mod_temp_manipulated_avg"]
        ]),
        errors="coerce",
    ).iloc[0]
    ...
    if drive_start_temp < dfc_start_temp:
        # DFC 시작 온도와 주행 시작 온도의 차이
        delta_temp = float(
            dfc_start_temp - drive_start_temp
        )

        # DFC 구간 전체의 평균 온도를 동일하게 낮춘다.
        df.loc[
            region_indices,
            "mod_temp_manipulated_avg",
        ] = (
            pd.to_numeric(
                df.loc[
                    region_indices,
        ...
        # 해당 구간에서 실제로 감소시킨 온도 기록
        df.loc[
            region_indices,
            "temp_delta",
        ] = delta_temp

        status = STATUS_MANIPULATED
