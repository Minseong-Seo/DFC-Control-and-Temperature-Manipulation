...
# 처리할 차량 모델을 리스트로 입력한다.
# 하나만 처리: ["EV6"]
# 두 모델 모두 처리: ["EV6", "Ioniq5"]
VEHICLE_MODELS = ["EV6", "Ioniq5"]

# 한 번만 실행할 때도 반드시 리스트 형태로 작성한다.
# 여러 값을 넣으면 아래 값들의 모든 조합을 순서대로 처리한다.
DFC_STAY_VALUES = [50, 60, 70, 80, 83, 85, 88, 90, 92]
TIME_MARGIN_MIN_VALUES = [0, 15, 30, 45, 60, 90, 120]

# 예시:
# DFC_STAY_VALUES = [50, 60, 70, 80, 83, 85, 88, 90, 92]
# TIME_MARGIN_MIN_VALUES = [0, 15, 30, 45, 60, 90, 120]
# → 총 63개 조건(9 x 7)을 처리한다.

# 특정 차량만 처리하려면 ID를 리스트로 입력한다.
DEBUG_DEVICES = [
    "01241228082",
    # "01241248842",
]
# 전체 차량을 처리하려면 위 리스트를 주석 처리하고 아래를 사용한다.
# DEBUG_DEVICES = None

# 특정 월만 처리하려면 "YYYY-MM"을 리스트로 입력한다.
TARGET_YMS = [
    "2023-08",
    # "2023-09",
]
# 전체 월을 처리하려면 위 리스트를 주석 처리하고 아래를 사용한다.
# TARGET_YMS = None
...
if not DFC_STAY_VALUES or not TIME_MARGIN_MIN_VALUES:
    raise ValueError(
        "DFC_STAY_VALUES와 TIME_MARGIN_MIN_VALUES는 비어 있을 수 없습니다."
    )


def normalize_filter_values(values):
    """None, 문자열 하나, 문자열 리스트를 동일한 리스트 형태로 맞춘다."""
    if values is None:
        return None
...
# 차량 모델과 DFC 조건에 따라 아래 경로가 자동으로 설정된다.
ROOT_DIR = Path("/Volumes/T7/DFC")

VEHICLE_MODEL = VEHICLE_MODELS[0]
INPUT_ROOT_DIR = ROOT_DIR / f"DFC_origin_{VEHICLE_MODEL}"
OUTPUT_ROOT_DIR = ROOT_DIR / f"Temp_manipulation_DFC_{VEHICLE_MODEL}"
...
def configure_paths(
    ...
    """현재 차량과 DFC 조건에 맞춰 입력·출력 경로를 설정한다."""
    ...
    DFC_FOLDER = DFC_SUFFIX.removeprefix("_")

    INPUT_ROOT_DIR = ROOT_DIR / f"DFC_origin_{VEHICLE_MODEL}"
    OUTPUT_ROOT_DIR = ROOT_DIR / f"Temp_manipulation_DFC_{VEHICLE_MODEL}"
    INPUT_DIR = INPUT_ROOT_DIR / DFC_FOLDER
    OUTPUT_DIR = OUTPUT_ROOT_DIR / DFC_FOLDER
    OUTPUT_DIR.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    log_device_name = (
        "all_devices"
        if DEBUG_DEVICES is None
        else "-".join(DEBUG_DEVICES)
    )
    LOG_PATH = OUTPUT_DIR / (
...
# 함수들이 참조할 기본 경로를 먼저 설정한다.
configure_paths(
    VEHICLE_MODELS[0],
    DFC_STAY_VALUES[0],
    TIME_MARGIN_MIN_VALUES[0],
)
